...
def score_case(
    ...
    if expected.get("requires_showboat_exec"):
        ...
        if min_sb is not None and sb_count < min_sb:
            score -= 2
            failures.append(
                f"expected at least {min_sb} showboat exec invocations, found {sb_count}"
            )
...
def score_all(
    ...
    for case in dataset:
        ...
        result = score_case(case, output, meta_cases.get(case["id"]))
...
def main():
    parser = argparse.ArgumentParser(
        description="Score showboat compliance benchmark outputs"
    )
    parser.add_argument(
